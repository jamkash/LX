...
/*
*ANSI为使计算机支持更多语言，通常使用 0x80~0xFFFF 范围的 2 个字节来表示 1 个字符。比如:汉字 '中' 在
中文操作系统中，使用 [0xD6,0xD0] 这两个字节存储。
*/
var VALID_ANSI = [ 874, 932, 936, 949, 950 ];  //有效ansi
...
function reset_ansi() { set_ansi(1252); }//重置ansi

//设置当前代码页
var set_cp = function(cp) { current_codepage = cp; set_ansi(cp); };
function reset_cp() { set_cp(1200); reset_ansi(); }//重置
...
XLSX.parse_zip = parse_zip;           //解析zip
XLSX.read = readSync; //xlsread
XLSX.readFile = readFileSync; //readFile
XLSX.readFileSync = readFileSync;     //同步读取文件
XLSX.write = writeSync;               //同步编写文件
XLSX.writeFile = writeFileSync;       //同步编写文件
XLSX.writeFileSync = writeFileSync;   //同步编写文件
XLSX.writeFileAsync = writeFileAsync; //异步编写文件
XLSX.utils = utils;
XLSX.SSF = SSF;
